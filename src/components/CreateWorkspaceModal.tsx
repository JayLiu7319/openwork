...
export default function CreateWorkspaceModal(props: {
  ...
  const options = () => [
    {
      id: "starter" as const,
      name: t('workspace.create.presets.starter.name'),
      desc: t('workspace.create.presets.starter.desc'),
    },
    {
      id: "minimal" as const,
      name: t('workspace.create.presets.minimal.name'),
      desc: t('workspace.create.presets.minimal.desc'),
    },
  ];

  const folderLabel = () => {
    const folder = selectedFolder();
    if (!folder) return t('workspace.create.folder.label');
  ...
  const folderSubLabel = () => {
    const folder = selectedFolder();
    if (!folder) return t('workspace.create.folder.subLabel');
    return folder;
  };
  ...
  const title = () => props.title ?? t('workspace.create.title');
  const subtitle = () => props.subtitle ?? t('workspace.create.subtitle');
  const confirmLabel = () => props.confirmLabel ?? t('workspace.create.submit');
    ...
    <div class="bg-zinc-900 border border-zinc-800 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
      ...
      <div class="p-6 flex-1 overflow-y-auto space-y-8">
        <div class="space-y-4">
          <div class="flex items-center gap-3 text-sm font-medium text-white">
            ...
            {t('workspace.create.steps.selectFolder')}
          </div>
          <div class="ml-9">
            <button
              type="button"
            ...
            >
              <div class="flex items-center gap-3 text-zinc-200">
                ...
                <Show
                  when={pickingFolder()}
                  fallback={<span class="text-xs text-zinc-500">{t('workspace.create.folder.change')}</span>}
                >
                  <span class="flex items-center gap-2 text-xs text-zinc-500">
                    <Loader2 size={12} class="animate-spin" />
                    {t('workspace.create.folder.opening')}
                  </span>
                </Show>
              </div>
            </button>
        ...
        <div class="space-y-4">
          <div class="flex items-center gap-3 text-sm font-medium text-white">
            ...
            {t('workspace.create.steps.choosePreset')}
          </div>
      ...
      <div class="p-6 border-t border-zinc-800 bg-zinc-950 flex justify-end gap-3">
        ...
        <Button
          ...
          title={!selectedFolder() ? t('workspace.create.folder.required') : undefined}
        >
          {confirmLabel()}
        </Button>
      </div>
