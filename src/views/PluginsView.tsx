      ...
      <div class="bg-zinc-900/30 border border-zinc-800/50 rounded-2xl p-5 space-y-4">
        <div class="flex items-start justify-between gap-4">
          <div class="space-y-1">
            ...
            <div class="text-xs text-zinc-500">{t('plugins.description')}</div>
          </div>
        ...
        <div class="flex flex-col gap-1 text-xs text-zinc-500">
          <div>{t('plugins.config.label')}</div>
          <div class="text-zinc-600 font-mono truncate">{props.pluginConfigPath ?? t('plugins.config.notLoaded')}</div>
        </div>

        <div class="space-y-3">
          <div class="text-xs font-medium text-zinc-400 uppercase tracking-wider">{t('plugins.suggested.title')}</div>
                    ...
                    <div class="flex items-start justify-between gap-4">
                      ...
                      <div class="flex items-center gap-2">
                        <Show when={isGuided()}>
                          ...
                          >
                            {isGuideOpen() ? t('plugins.suggested.hideSetup') : t('plugins.suggested.setup')}
                          </Button>
                        </Show>
                        <Button
                        ...
                        >
                          {isInstalled() ? t('plugins.suggested.added') : t('plugins.suggested.add')}
                        </Button>
                      </div>
                    </div>
                          ...
                          {(step, idx) => (
                            <div class="space-y-1">
                              <div class="text-xs font-medium text-zinc-300">
                                {t('plugins.guide.step', { idx: idx() + 1, title: step.title })}
                              </div>
                              ...
                              <Show when={step.url}>
                                <div class="text-xs text-zinc-500">
                                  {t('plugins.guide.open')} <span class="font-mono text-zinc-400">{step.url}</span>
                                </div>
                              </Show>
                              <Show when={step.path}>
                                <div class="text-xs text-zinc-500">
                                  {t('plugins.guide.path')} <span class="font-mono text-zinc-400">{step.path}</span>
                                </div>
                              </Show>
                            </div>
                          )}
            ...
            <For each={props.pluginList}>
              {(pluginName) => (
                <div class="flex items-center justify-between rounded-xl border border-zinc-800/60 bg-zinc-950/40 px-4 py-2.5">
                  ...
                  <div class="text-[10px] uppercase tracking-wide text-zinc-500">{t('plugins.status.enabled')}</div>
                </div>
              )}
            </For>
          </div>
        ...
        <div class="flex flex-col gap-3">
          <div class="flex flex-col md:flex-row gap-3">
            <div class="flex-1">
              <TextInput
                label={t('plugins.addPlugin.label')}
                placeholder={t('plugins.addPlugin.placeholder')}
                value={props.pluginInput}
                onInput={(e) => props.setPluginInput(e.currentTarget.value)}
                hint={t('plugins.addPlugin.hint')}
              />
            </div>
            <Button
              variant="secondary"
