...
export function createTemplateState(options: {
  ...
  async function saveTemplate() {
    ...
    if (!draft.title || !draft.prompt) {
      options.setError(t("errors.operation.failed"));
      return;
    }
  ...
  async function runTemplate(template: WorkspaceTemplate) {
    ...
    try {
      ...
      const message = e instanceof Error ? e.message : t("errors.operation.unknown");
      options.setError(addOpencodeCacheHint(message));
    } finally {
      options.setBusy(false);
    }
    ...
    try {
      ...
      const parseTemplateContent = (raw: string, fallbackId: string) => {
        ...
        if (parsedFrontmatter) {
          ...
          const title = typeof meta.title === "string" ? meta.title : t("common.labels.title");
        ...
        const title = typeof parsed.title === "string" ? parsed.title : t("common.labels.title");
