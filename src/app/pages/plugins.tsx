    ...
    <section class="space-y-6">
      <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-4">
        ...
        <div class="flex flex-col gap-1 text-xs text-gray-10">
          ...
          <div class="text-gray-7 font-mono truncate">{props.pluginConfigPath ?? t('plugins.config.notLoaded')}</div>
        </div>

        <div class="space-y-3">
          <div class="text-xs font-medium text-gray-11 uppercase tracking-wider">{t('plugins.suggested.title')}</div>
                            ...
                            <div class="space-y-1">
                              ...
                              <Show when={step.url}>
                                <div class="text-xs text-gray-10">
                                  {t('plugins.guide.open')} <span class="font-mono text-gray-11">{step.url}</span>
                                </div>
                              </Show>
                              <Show when={step.path}>
                                <div class="text-xs text-gray-10">
                                  {t('plugins.guide.path')} <span class="font-mono text-gray-11">{step.path}</span>
                                </div>
                              </Show>
                            </div>
                          )}
            ...
            <For each={props.pluginList}>
              {(pluginName) => (
                <div class="flex items-center justify-between rounded-xl border border-gray-6/60 bg-gray-1/40 px-4 py-2.5">
                  ...
                  <div class="text-[10px] uppercase tracking-wide text-gray-10">{t('plugins.status.enabled')}</div>
                </div>
              )}
            </For>
          </div>
