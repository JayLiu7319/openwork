...
export default function OnboardingView(props: OnboardingViewProps) {
  ...
  const engineStatusLabel = () => {
    if (props.engineDoctorFound == null || props.engineDoctorSupportsServe == null) {
      return t('onboarding.engine.checking');
    }
    if (!props.engineDoctorFound) return t('onboarding.engine.notFound');
    if (!props.engineDoctorSupportsServe) return t('onboarding.engine.needsUpdate');
    if (props.engineDoctorVersion) return `OpenCode ${props.engineDoctorVersion}`;
    return t('onboarding.engine.ready');
  };
          ...
          <div class="z-10 flex flex-col items-center gap-6">
            ...
            <div class="text-center">
              <h2 class="text-xl font-medium mb-2">
                {props.mode === "host" ? t('onboarding.connecting.hostTitle') : t('onboarding.connecting.clientTitle')}
              </h2>
              <p class="text-gray-10 text-sm">
                {props.mode === "host"
                  ? t('onboarding.connecting.hostDescription')
                  : t('onboarding.connecting.clientDescription')}
              </p>
          ...
          <div class="max-w-lg w-full z-10 space-y-6">
            ...
            <details class="rounded-2xl border border-gray-6 bg-gray-1/60 px-4 py-3">
              ...
              <div class="pt-3 space-y-3">
                      ...
                      <div class="mt-4 space-y-2">
                        ...
                        <div class="flex gap-2 pt-2">
                          ...
                          <Button variant="outline" onClick={props.onShowSearchNotes} disabled={props.busy}>
                            {t('onboarding.engine.searchNotes')}
                          </Button>
                        </div>
                      </div>
                    </Show>
                ...
                <Show when={engineDoctorAvailable()}>
                  <div class="text-xs text-gray-7">{t('onboarding.engine.readyToStart')}</div>
                </Show>
                ...
                >
                  <div class="rounded-xl bg-gray-1/40 border border-gray-6 p-3 space-y-3 text-xs text-gray-10">
                    <Show when={props.engineDoctorResolvedPath}>
                      <div>
                        <div class="text-[11px] text-gray-8">{t('onboarding.engine.resolvedPath')}</div>
                        <div class="font-mono break-all">{props.engineDoctorResolvedPath}</div>
                      </div>
                    </Show>
                    <Show when={props.engineDoctorVersion}>
                      <div>
                        <div class="text-[11px] text-gray-8">{t('onboarding.engine.version')}</div>
                        <div class="font-mono">{props.engineDoctorVersion}</div>
                      </div>
                    </Show>
                    <Show when={props.engineDoctorNotes.length}>
                      <div>
                        <div class="text-[11px] text-gray-8">{t('onboarding.engine.searchNotes')}</div>
                    ...
                    <Show when={serveHelpOutput()}>
                      <div>
                        <div class="text-[11px] text-gray-8">{t('onboarding.engine.serveHelp')}</div>
        ...
        <div class="min-h-screen flex flex-col items-center justify-center bg-gray-1 text-gray-12 p-6 relative">
          ...
          <div class="max-w-md w-full z-10 space-y-8">
            <div class="text-center space-y-2">
              ...
              <p class="text-gray-11 text-sm leading-relaxed">
                {t('onboarding.client.description')}
              </p>
            </div>

            <div class="space-y-4">
              <TextInput
                label={t('onboarding.client.urlLabel')}
                placeholder={t('onboarding.client.urlPlaceholder')}
                value={props.baseUrl}
                onInput={(e) => props.onBaseUrlChange(e.currentTarget.value)}
              />
              <TextInput
                label={t('onboarding.client.directoryLabel')}
                placeholder={t('onboarding.client.directoryPlaceholder')}
                value={props.clientDirectory}
                onInput={(e) => props.onClientDirectoryChange(e.currentTarget.value)}
                hint={t('onboarding.client.directoryHint')}
              />
          ...
          <div class="max-w-xl w-full z-10 space-y-12">
            <div class="text-center space-y-4">
              <div class="flex items-center justify-center gap-3 mb-6">
                ...
                <h1 class="text-3xl font-bold tracking-tight text-gray-12">{t('onboarding.title')}</h1>
              </div>
              <h2 class="text-xl text-gray-11">{t('onboarding.mode.subtitle')}</h2>
            </div>
            ...
            <div class="space-y-4">
              ...
              >
                ...
                <div>
                  <h3 class="text-xl font-medium text-gray-12 mb-2">{t('onboarding.mode.host.title')}</h3>
                  <p class="text-gray-10 text-sm leading-relaxed mb-4">
                    {t('onboarding.mode.host.description')}
                  </p>
              ...
              <Show when={props.engineRunning && props.engineBaseUrl}>
                <div class="rounded-2xl bg-gray-2/40 border border-gray-6 p-5 flex items-center justify-between">
                  <div>
                    <div class="text-sm text-gray-12 font-medium">{t('onboarding.mode.engineRunning.title')}</div>
                    <div class="text-xs text-gray-10">{t('onboarding.mode.engineRunning.description')}</div>
                    <Show when={props.developerMode}>
                  ...
                  <Button variant="secondary" onClick={props.onAttachHost} disabled={props.busy}>
                    {t('onboarding.mode.engineRunning.button')}
                  </Button>
                </div>
              </Show>
              ...
              <div class="flex items-center gap-2 px-2 py-1">
                ...
                >
                  ...
                  {t('onboarding.mode.rememberChoice')}
                </button>
              </div>
