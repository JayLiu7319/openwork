                              ...
                              <div class={idx() === groups().length - 1 ? "" : groupSpacing()}>
                                ...
                                <Show when={group.kind === "steps"}>
                                  <div class={isUser() ? "mt-2" : "mt-3 border-t border-gray-6/60 pt-3"}>
                                    <button
                                      class={`flex items-center gap-2 text-xs ${isUser() ? "text-gray-10 hover:text-gray-11" : "text-gray-10 hover:text-gray-12"
                                        }`}
                    ...
                    <div class="px-4 pb-4 pt-1">
                      <div class="flex items-center gap-2">
                        <For each={progressDots()}>
                          {(done) => (
                            <div
                              class={`h-6 w-6 rounded-full border flex items-center justify-center ${done ? "border-green-6 text-green-11" : "border-gray-7 text-gray-8"
                                }`}
                            >
                              <Show when={done}>
                                <Check size={14} />
                      ...
                      <div class="mt-2 text-xs text-gray-10">{t('session.steps.unfold')}</div>
                    </div>
                  </Show>
                </div>
              </Show>
                ...
                <Show when={props.expandedSidebarSections.artifacts}>
                  <div class="px-4 pb-4 pt-1 space-y-3">
                    <Show
                      ...
                      fallback={<div class="text-xs text-gray-7">{t('session.artifacts.empty')}</div>}
                    >
              ...
              <div class="rounded-2xl border border-gray-6 bg-gray-1/60">
                ...
                <Show when={props.expandedSidebarSections.context}>
                  <div class="px-4 pb-4 pt-1 space-y-4">
                      ...
                      <div>
                        ...
                        <div class="mt-2 space-y-2">
                          <Show
                            ...
                            fallback={
                              <div class="text-xs text-gray-7">{props.activePluginStatus ?? t('session.context.noPlugins')}</div>
                            }
                          >
                    ...
                    <div>
                      <div class="text-xs text-gray-10">{t('session.context.workingFiles')}</div>
                      <div class="mt-2 space-y-2">
                        <Show
                          when={props.workingFiles.length}
                          fallback={<div class="text-xs text-gray-7">{t('session.context.noneYet')}</div>}
                        >
              ...
              <div class="p-2 pt-6 pb-3 px-4">
                <Show when={props.showTryNotionPrompt}>
                  ...
                  >
                    ...
                    <span class="text-xs text-green-12 font-medium">{t('session.input.insertPrompt')}</span>
                  </button>
                </Show>
                          ...
                          >
                            <For each={commandMatches()}>
                              {(command, idx) => (
                                <button
                                  type="button"
                                  class={`w-full flex items-start gap-3 rounded-xl px-3 py-2 text-left transition-colors ${idx() === commandIndex()
            ...
            <div class="bg-gray-2 border border-amber-7/30 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
              <div class="p-6">
                <div class="flex items-start gap-4 mb-4">
                  ...
                  <div>
                    <h3 class="text-lg font-semibold text-gray-12">{t('session.permissions.required')}</h3>
                    <p class="text-sm text-gray-11 mt-1">{t('session.permissions.requesting')}</p>
                  </div>
                </div>

                <div class="bg-gray-1/50 rounded-xl p-4 border border-gray-6 mb-6">
                  <div class="text-xs text-gray-10 uppercase tracking-wider mb-2 font-semibold">{t('session.permissions.permission')}</div>
                  <div class="text-sm text-gray-12 font-mono">{props.activePermission?.permission}</div>

                  <div class="text-xs text-gray-10 uppercase tracking-wider mt-4 mb-2 font-semibold">{t('session.permissions.scope')}</div>
                  ...
                  <Show when={Object.keys(props.activePermission?.metadata ?? {}).length > 0}>
                    <details class="mt-4 rounded-lg bg-gray-1/20 p-2">
                      <summary class="cursor-pointer text-xs text-gray-11">{t('session.permissions.details')}</summary>
